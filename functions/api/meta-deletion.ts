...
async function parseSignedRequest(signedRequest: string, secret: string): Promise<{ user_id?: string } | null> {
  try {
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  if (!body.signed_request) return Response.json({ error: 'Missing signed_request' }, { status: 400 })
