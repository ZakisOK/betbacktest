...
import type { Action, BetSide, Modifiers, ProgressionMethod, Rule, Trigger } from "../types";
...
function parseProgression(s: string): ProgressionMethod {
  ...
  if (u.includes("d'alembert") || u.includes("dalembert") || u.includes("alembert"))
    return "dalembert";
    ...
    const rules: Rule[] = [
      makeRule({
        ...
        trigger: {
          type: "streak",
          side: streakSide,
          direction: "consecutive_wins",
          min_length: streakCount,
        },
    ...
    const rules: Rule[] = [
      makeRule({
        ...
        trigger: {
          type: "streak",
          side: "Any",
          direction: isLoss ? "consecutive_losses" : "consecutive_wins",
          min_length: count,
        },
      ...
      rules.push(
        makeRule({
          ...
          trigger: {
            type: "streak",
            side: "Any",
            direction: isLoss ? "consecutive_wins" : "consecutive_losses",
            min_length: 1,
          },
          action: { type: "reset_progression", reset_to: 1 },
          modifiers: { shoe_reset: "carry" },
    ...
    return [
      makeRule({
        ...
        trigger: {
          ...
          condition: isBelow ? "bankroll_below" : "bankroll_above",
          threshold: amt,
        },
  ...
  (t) => {
    ...
    return [
      makeRule({
        ...
        action: {
          type: "place_bet",
          side: betSide === "Any" ? "Banker" : (betSide as BetSide),
          unit_size: units,
        },
      }),
    ];
    ...
    return [
      makeRule({
        ...
        trigger: {
          type: "streak",
          side: "Any",
          direction: isWin ? "consecutive_wins" : "consecutive_losses",
          min_length: 1,
        },
        action: { type: "reset_progression", reset_to: 1 },
        modifiers: { shoe_reset: "carry" },
      ...
      .map(
        (r) =>
          ({
            ...
            priority: 0,
            enabled: true,
            modifiers: { shoe_reset: "reset" as const },
            ...r,
          }) as Rule
      );
