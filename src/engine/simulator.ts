...
import { generateShoe, dealHand, mulberry32 } from './baccarat'
...
function getProgressionBet(action: Action, state: SimState): number {
  ...
  switch (action.method) {
    ...
    case 'oscars_grind': {
      const { profit, base: ob } = state.progression.oscars
...
function updateState(
  ...
  if (decision.side && decision.amount > 0) {
    // Find the rule that produced this decision to get its action
    // For now, update progression based on whether we won or lost
    const won = decision.side === outcome || (outcome === 'Tie' && config.tie_handling === 'push')
    const lost = pnl < 0
    if (won && pnl > 0) {
      // We won — advance/reset progression
      state.current_unit = Math.max(state.base_unit, state.current_unit)
    } else if (lost) {
      // We lost
    }
  }
